...
/**
 * 格式化字符串 用法: .formatString("{0}-{1}","a","b");
 */
...
utils.buildFormValues = function(formId) {
	var o = {};
...
utils.formatListData = function(data, prefix) {
	if (!data || !data.length)
		return [];
	var result = {};
	$.each(data, function(i, d) {
		for ( var p in d) {
			var property = prefix + "[" + i + "]." + p;
			result[property] = d[p];
		}
	});
	return result;
...
utils.buildSearhRules = function(formId) {
	var rules = [];
	$("#" + formId).find("input[name]").each(function(i, input) {
		var value = $(input).val();
		...
		if (value) {
			var op = "like";
			var sourceBox = $("input[textboxname=" + input.name + "]");
			if (sourceBox.attr("operation")) {
				op = sourceBox.attr("operation");
			}
			var rule = utils.getItemFromList('field', input.name, rules);
			if (!rule) {
				rules.push({
					field : input.name,
					op : op,
...
utils.getItemFromList = function(keyField, value, list) {
	var result = null;
...
utils.formatDate = function(v, format) {
	...
	if (typeof v === 'string') {
		if (v.indexOf("/Date(") > -1)
			...
			d = new Date(Date.parse(v.replace(/-/g, "/").replace("T", " ")
					.split(".")[0]));// .split(".")[0]
...
utils.copyProperty = function(obj, sourcePropertyName, newPropertyName,
		overWrite) {
	if (obj instanceof Array
			|| Object.prototype.toString.call(obj) === "[object Array]") {
		for ( var k in obj)
		...
		if (sourcePropertyName instanceof Array
				|| Object.prototype.toString.call(sourcePropertyName) === "[object Array]") {
			for ( var i in sourcePropertyName) {
				utils.copyProperty(obj, sourcePropertyName[i],
						newPropertyName[i]);
			}
