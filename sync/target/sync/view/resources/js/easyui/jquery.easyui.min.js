...
(function($) {
	$.parser = {
		...
		plugins : [ "draggable", "droppable", "resizable", "pagination",
				...
				"dialog", "form","importwin","ajaxupload","repeater"],
		parse : function(_2) {
			var aa = [];
...
(function($) {
	...
	function _178(_179, id) {
		...
		return _11c(_179,  opts.valueField || "id", id);
	}
	;
		...
		dc.body1.add(dc.body2).unbind().bind("mouseover", function(e) {
			...
			if (tt.parent().hasClass("datagrid-cell-check")) {
				...
				if (td.length) {
					...
					opts.onClickCell.call(_5ba, _5c7, _5c8, row[_5c8],tt);
				}
				...
				opts.onClickRow.call(_5ba, _5c7, row,tt);
			}
		}).bind("dblclick", function(e) {
			var tt = $(e.target);
			var tr = tt.closest("tr.datagrid-row");
...
(function($) {
	...
	function _8f9(_8fa, url, _8fb, _8fc) {
		...
		_8fb = _8fb || {};
		_8fb = $.extend({}, opts.queryParams, _8fb);
...
(function($) {
	...
	function _930(_931) {
		 var _932 = $.data(_931, "combotree");
	        ...
	        var textField = opts.textField || 'Name';
	        if (opts.multiple) {
	            var _933 = tree.tree("getChecked");
	            for (var i = 0; i < _933.length; i++) {
	                vv.push(_933[i][valueField]);
	                ss.push(_933[i][textField]);
	            }
	        } else {
	            var node = tree.tree("getSelected");
	            if (node) {
	                vv.push(node[valueField]);
	                ss.push(node[textField]);
	            }
	        }
...
(function($){
	$.fn.repeater = function(options, params) {
		if (typeof options == "string") {
			return $.fn.repeater.methods[options](this, params);
		}
		options = options || {};
		return this.each(function() {
			var data = $.data(this, "repeater");
			var opts = {};
			if (data) {
				opts = $.extend(data.options, options);
				data.options = opts;
			} else {
				opts = $.extend({}, $.extend({}, $.fn.repeater.defaults, {
					queryParams : {}
				}), $.fn.repeater.parseOptions(this), options);
				if (!opts.columns) {
	...
	$.fn.repeater.methods = {
			getData : function(jq) {
				return $.data(jq[0], "repeater").data;
			},
			options : function(jq) {
				return $.data(jq[0], "repeater").options;
			},
			appendRow: function(jq,row)
			{
				var rows=$.data(jq[0], "repeater").data;
				rows.push(row);
				renderRows(jq[0],[row]);
			},
			removeData: function(jq,index)
			{
				var rows=$.data(jq[0], "repeater").data;
				rows.splice(index,1);
			},
	...
	function renderRows(target, data) {
		var opts = $.data(target, "repeater").options;
		if(data==null)
			return;
		$.each(data, function(i, row) {
			var tempText=$("#"+opts.templateId).html();
			$.each(opts.columns,function(j,column){
				var value=row[column.field] || '';
				tempText=tempText.replace("##"+column.field+"##",value);
			});
			$(target).append(tempText);
		});
		$.parser.parse(target);
	}
	function loadData(target, params) {
		...
		function loadRemote() {
			...
			$.ajax({
				...
				dataType : "json",
				success : function(data) {
					$.data(target, "repeater").data=data;
					if(data==null || !data.length)
					{
						for(var i=0;i<opts.defaultRowCount;i++)
						{
							data.push({});
						}
					}
					renderRows(target, data);
				}
			});
		}
		;
	};
	$.fn.repeater.parseOptions = function(target) {
		var t = $(target);
