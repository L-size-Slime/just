...
(function($) {
	var currTarget;
	$(function() {
		$(document).unbind('.edatagrid').bind(
				'mousedown.edatagrid',
				function(e) {
	...
	function buildGrid(target) {
				...
				$.extend({}, opts, {
					onDblClickCell : function(index, field, value) {
						...
						if (opts.onDblClickCell) {
							opts.onDblClickCell.call(target, index, field,
									value);
						}
					},
					...
					onAfterEdit : function(index, row) {
						opts.editIndex = -1;
						var url = row.isNewRecord ? opts.saveUrl
								: opts.updateUrl;
						if (url) {
							$.post(url, row, function(data) {
								...
								if (opts.tree) {
									...
									if (node) {
										...
										var pnode = t.tree('find',
												row[opts.treeParentField]);
										t.tree('append', {
											parent : (pnode ? pnode.target
													: null),
											data : [ {
												id : idValue,
					...
					onBeforeLoad : function(param) {
						...
						if (opts.tree) {
							...
							param[opts.treeParentField] = node ? node.id
									: undefined;
						}
					},
		...
		function focusEditor(field) {
			...
			if (editor) {
				focusEditorInput(editor.target[0]);
			} else {
				var editors = $(target).datagrid('getEditors', opts.editIndex);
				if (editors.length) {
					focusEditorInput(editors[0].target[0]);
				}
			}
		}

		function focusEditorInput(target) {
	...
	$.fn.edatagrid = function(options, param) {
		...
		return this.each(function() {
			...
			if (state) {
				...
				$.data(this, 'edatagrid', {
					options : $.extend({}, $.fn.edatagrid.defaults,
							$.fn.edatagrid.parseOptions(this), options)
				});
			}
			buildGrid(this);
		});
		...
		addRow : function(jq, index) {
			return jq
					.each(function() {
						...
						if (opts.tree) {
							var node = $(opts.tree).tree('getSelected');
							rows[opts.editIndex][opts.treeParentField] = (node ? node.id
									: 0);
						}
			...
			return jq
					.each(function() {
						...
						if (index == undefined) {
							...
							var rowIndexes = $.isArray(index) ? index
									: [ index ];
							for (var i = 0; i < rowIndexes.length; i++) {
								var row = opts.finder.getRow(this,
										rowIndexes[i]);
								if (row) {
									rows.push(row);
								}
						...
						$.messager.confirm(opts.destroyMsg.confirm.title,
								opts.destroyMsg.confirm.msg, function(r) {
									if (r) {
											...
											.post(
													...
													function(data) {
														...
														if (pager.length
															...
															dg
																	.datagrid('options').pageNumber = pager
	...
	$.fn.edatagrid.defaults = $.extend({}, $.fn.datagrid.defaults, {
		...
		autoSave : false, // auto save the editing row when click out of
							// datagrid
		...
		treeDndUrl : null, // to process the drag and drop operation, return
							// {success:true}
		treeTextField : 'name',
