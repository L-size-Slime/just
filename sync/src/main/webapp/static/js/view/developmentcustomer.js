...
function ChangeProvince() {
	...
	if (provinceId != null && provinceId != "" && provinceId != "0") {
		com.ajax({
			url : '/city/loadDevelopmentCity?id=' + provinceId,
			success : function(d) {
				$("#cityId").empty();
				$("#countyId").empty();
				if (com.isBlank(d)) {
...
function ChangeCity() {
	...
	com.ajax({
		url : '/county/loadDevelopmentCounty?id=' + cityId,
		success : function(d) {
			$("#countyId").empty();
			if (com.isBlank(d)) {
			} else {
...
function ChangeCounty() {
	...
	com.ajax({
		url : '/town/loadDevelopmentTown?id=' + countyId,
		success : function(d) {
			$("#townId").empty();
			if (com.isBlank(d)) {
			} else {
...
function openSearch() {
	...
	com.openWin("客户名称检索,关键字:" + cusName, "/customer/querybycusnm?cusNm="
			+ cusName, "800px", "500px");
...
var contactid = 1;
function saveContact(masterNm, deptNm, dutyClassID, dutyID, dutyClassName,
		dutyName, duty, mobile, tel, fax, ynreceive, sex, email, remark) {
	var row = {};
	row.id = contactid;
...
function OnSelectCustomer() {
	winIndex = com.openWin("选择关联的客户", "/customer/selectcustomer", "800px",
			"500px");
}
